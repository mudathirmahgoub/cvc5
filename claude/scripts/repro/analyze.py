#!/usr/bin/env python3
"""Analyze a campaign CSV against the comparison sheet -- the exact reasoning
used to turn raw run logs into the report's numbers.

Usage:
    analyze.py <config>.csv                       # solved counts + discrepancy audit
    analyze.py <config>.csv --vs <other>.csv      # head-to-head (+solved/-lost, timing)

Reports, for the campaign CSV:
  * solved / sat / unsat / timeout counts;
  * answer DISCREPANCIES against every recorded reference column -- the
    soundness audit. A discrepancy = both sides give a definite (sat|unsat)
    answer and they differ.

Honesty note printed automatically: the only nonzero discrepancies in the
final run are against the plain `sqlsolver` column, on 5 MAPA instances where
SQLSolver reports SAT. Those are SQLSolver unsoundness, not ours: the
independent eager cvc5 encoding (`cvc5 lia`) and `sls` agree with our `unsat`,
and the corrected `modified_sqlsolver` downgrades all 5 to `unknown`.
"""
import sys, pandas as pd

COMPARISON = "/home/mudathir/all/paper-fmcad26-liastar/scripts/comparison.csv"
DEFI = ["sat", "unsat"]
