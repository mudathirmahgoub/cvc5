...
\fn{collectRelsInfo}, or twice, as the code did until recently?}
The answer is \emph{once}: the second call is not merely redundant work, it
silently destroys the closure edges that the first call's rule application had
just recorded, and makes the solver diverge on problems it should refute. The
evidence was in the repository already --- the in-tree regression
\file{regress0/rels/rel\_tc\_7.cvc.smt2} was \emph{disabled} by the very commit
that introduced the second call, and it is re-enabled by this work (timeout
$\to$ \textsf{unsat} in 0.01\,s). A patch, a minimal repro, a differential fuzz
test against an exact oracle, and a measurement on all 295
\code{sets}\slash\code{rels} regression benchmarks are included.

\medskip\noindent
\textbf{Status.} Part~\ref{part:tc} was first written against
\code{8dd659a3a}, where \fn{buildTCGraphForRel} had two \emph{unguarded} call
sites. While it was being written, PR~\#12793 (\code{c851f83bf}) fixed the same
destructive overwrite differently, by \emph{guarding} both call sites. The part
has been updated to compare against that baseline as well, and it is careful to
separate the two claims: against the unguarded code the one-call patch turns
divergence into instant refutations (Section~\ref{sec:measure}), while against
the guarded code it is behaviourally and performance-neutral
(Section~\ref{sec:measure-vs-guarded}) and the case for it is the invariant it
establishes, not a speedup. Section~\ref{sec:mergehazard} records why the two
fixes must not be combined half-way. The implementation is commit
\code{8bcdf3239} on branch \code{build-trc-graph-once}; line numbers throughout
refer to it unless stated otherwise.
\end{abstract}
...
\ref{inv:life} have always held. Invariant~\ref{inv:complete} did
\textbf{not}, until the two fixes discussed in the next part; with Phase~0 it
holds by construction, because the only assignment to
\code{d\_tcr\_tcGraph}$[T]$ happens before any rule runs and everything after it
is an insertion. That is the subject of the next part.
...
At commit \code{8dd659a3a}, where this investigation started,
\fn{buildTCGraphForRel} had two call sites:
...
& Site & Guard\\
\midrule
$\bigstar_1$ & \fn{applyTCRule}, line 644 &
...
$\bigstar_2$ & \fn{check()} Phase~B, line 189 & \emph{none}\\
\bottomrule
\end{tabular}
\end{center}

\noindent
Whenever a closure term $T$ has an asserted member and its base relation $r$
has one too, $\bigstar_2$ always fires and $\bigstar_1$ fires as well provided
$r$ happens to be its own representative --- so in the common case both run in
the same \fn{check()} call, $\bigstar_1$ first (Phase~A) and $\bigstar_2$ second
(Phase~B). (When $r$ is not its own representative, only $\bigstar_2$ runs, and
Phase~A reasons without a graph at all; see Section~\ref{sec:tcrule}.) The question is whether to replace them with a single unconditional
pass immediately after \fn{collectRelsInfo}.

There are therefore three configurations to keep apart, and the rest of this
part is careful about which one it is measuring against:

\begin{center}\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}llp{6.9cm}@{}}
\toprule
Name & Commit & \fn{buildTCGraphForRel} call sites\\
\midrule
\emph{unguarded} & \code{8dd659a3a} &
  $\bigstar_1$ (guarded) and $\bigstar_2$ (\emph{no} guard): the second call
  overwrites the first call's augmented graph\\
\emph{guarded} & \code{c851f83bf} (PR~\#12793) &
  $\bigstar_1$ and $\bigstar_2$, both guarded on \code{d\_rel\_nodes} and
  \code{d\_rRep\_tcGraph}, so at most one of them builds per base
  representative\\
\emph{once} & \code{8bcdf3239} &
  Phase~0 only, unconditional, one call per closure term, before any rule\\
\bottomrule
\end{tabular}
\end{center}

\subsection{What PR \#12793 changed}\label{sec:12793}

PR~\#12793 diagnosed exactly the overwrite described below and fixed it by
adding to $\bigstar_2$ the guard that $\bigstar_1$ already had:
...
\noindent
That is sufficient, and it comes with the regression
\file{regress1/rels/tc-overwrite-bug.smt2} --- benchmark~A of
Section~\ref{sec:repro} in all but name. It is worth being explicit that the
\emph{guarded} configuration is correct on everything measured here
(Section~\ref{sec:measure-vs-guarded}), so what follows is not a bug report
against it. Two properties still distinguish it from \emph{once}:

\begin{itemize}[leftmargin=*,itemsep=2pt]
\item \textbf{Which call builds is decided by iteration order.} Both maps in the
  guard double as ``already built'' markers, so the outcome depends on whether
  Phase~A reached $T$ before Phase~B did. Correctness is preserved by the fact
  that Phase~A always runs first and that $\bigstar_1$ builds before it augments
  --- a two-site argument, where \emph{once} needs no argument at all.
\item \textbf{The guard is keyed by base representative, not by term.} If two
  ...
  $\bigstar_1$ later inserts. Invariant~\ref{inv:complete} still fails for that
  term. This is benign --- the two closure terms are congruent, so the
  memberships derived for the first propagate --- and it is unobservable in the
  test suite, but it is a weaker statement than the invariant.
\end{itemize}
...
\begin{observation}[The second call is destructive]\label{obs:destroy}
Between $\bigstar_1$ and $\bigstar_2$, \fn{applyTCRule} adds edges to
\code{d\_tcr\_tcGraph}$[T]$ for the asserted members of $T$ itself
(Algorithm~\ref{alg:tcrule}, line 8). $\bigstar_2$ then executes
\code{d\_tcr\_tcGraph[tc\_rel] = rel\_tc\_graph}, a whole-map assignment.
\textbf{All of those edges are discarded}, together with their explanations.
\end{observation}

\noindent
So the second call is not a redundant recomputation of the same thing. It is a
recomputation of a \emph{smaller} thing that overwrites a larger one. The
guard is \code{if (members.size() > 0)}, so the damage is done exactly when the
base relation has at least one asserted member; when $r$ has no members the
overwrite does not happen and the closure edges survive. That explains why the
bug is not visible on most of the transitive-closure regressions, which assert
memberships in $\tc{r}$ or in $r$ but rarely both.
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \begin{scope}[xshift=5.2cm]
    \node[lbl] at (1.1,1.2) {after $\bigstar_2$ (current code)};
    \node[n] (a2) at (0,0) {$1$}; \node[n] (b2) at (1.1,0) {$2$};
    \node[n] (c2) at (2.2,0) {$3$};
    \draw[base] (b2)--(c2);
  ...
  \end{scope}
\end{tikzpicture}
\caption{Effect of the second \fn{buildTCGraphForRel} call on
$G_{\tc{x}}$ for the benchmark of Section~\ref{sec:repro}.}
\end{figure}

\begin{observation}[Cost]
The wasted work of $\bigstar_2$ is $O(|\mathit{members}(r)|)$ node
constructions and map insertions per closure term per full-effort round ---
including one \code{RelsUtils::constructPair} \code{APPLY\_CONSTRUCTOR} node
per member, which goes through the node manager's hash-consing table. It is not
dominant, but it is pure loss.
\end{observation}

\section{A benchmark that separates the two}\label{sec:repro}
...
\noindent
With the second call in place, $G_{\tc{x}}$ is reduced to the single edge
$2\to3$, \fn{doTCInference} only re-derives $\tup{2,3}\in\tc{x}$, and the solver
falls back on repeatedly splitting the \textsc{TClos Down} disjunction of
\fn{applyTCRule} over freshly reachable Skolems --- it diverges instead of
refuting.

Four variations pin the trigger down exactly. All were run on the same Debug
build with a 20\,s limit.
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
\item \textbf{B} is the control for the \code{members.size() > 0} guard. Moving
  the one base membership into the closure makes $r$ memberless, so
  $\bigstar_2$ does not overwrite --- and current \code{main} refutes it
  instantly. The diagnosis really is the overwrite.
\item \textbf{C} is Example~3 of~\cite{MRT17} verbatim; both configurations
  handle it, because it needs no closure-only edges.
\item \textbf{D} checks that the patch does not \emph{over}-infer: there is no
  path from 1 to 4, the problem is satisfiable, and both configurations say
  \textsf{sat}.
\item \textbf{E} shows the effect is not specific to a two-edge path.
\end{itemize}

\subsection{The in-tree regression that PR \#7515 had to disable}
\label{sec:reltc7}

The strongest evidence is already in the repository.
...
\noindent
and \file{test/regress/cli/CMakeLists.txt} contains, in
...
line was added by PR~\#7515 --- the same commit that removed the Phase~B guard
and thereby created the second \fn{buildTCGraphForRel} call. The PR description
says as much: \emph{``It disables a regression that times out after these
fixes.''}
...
\midrule
\emph{unguarded} (\code{8dd659a3a}) & \textbf{timeout} & $>300$\,s\\
\emph{guarded} (\code{c851f83bf})   & \textbf{unsat}   & 0.01\,s\\
\emph{once} (\code{8bcdf3239})      & \textbf{unsat}   & 0.01\,s\\
\bottomrule
\end{tabular}
\end{center}

\noindent
This benchmark has the same shape as A: the join and the outer closure put
memberships into both $x$'s class and $\tc{x}$'s class, so $\bigstar_2$ throws
the closure edges away. Either fix makes it pass, so the credit for reviving it
belongs to whichever landed first, which is PR~\#12793 --- but nobody had noticed
that it was revived. Commit \code{8bcdf3239} therefore also moves it out of
\code{regression\_disabled\_tests} into \code{regress\_0\_tests}, where it now
passes in 0.13\,s under \fn{ctest} and also under \code{--check-proofs},
...
\code{regress1/rels/garbage\_collect.cvc.smt2} still times out in every
configuration.
...
\item Delete $\bigstar_2$ from Phase~B, \emph{including its \#12793 guard}: with
  Phase~0 in place the guard is not a safety net, it is the last thing keeping a
  destructive call alive (Section~\ref{sec:mergehazard}).
...
\code{d\_tcr\_tcGraph}. Building afterwards would both weaken the guard and be
overwritten again.

A pleasant side effect: the representative mismatch noted in
Section~\ref{sec:tcrule} (\code{find(tc\_rel[0])} versus
\code{getRepresentative(tc\_rel[0])}) disappears, because the new pass does not
test it at all.

\subsection{A smaller alternative, and why it is worse}

One could keep the lazy build and merely stop $\bigstar_2$ from clobbering,
e.g.\ by guarding Phase~B with
\code{if (d\_tcr\_tcGraph.find(*term\_it) == d\_tcr\_tcGraph.end())} or by making
\fn{buildTCGraphForRel} merge instead of assign. That fixes the divergence, but
it is strictly weaker than the up-front pass:

\begin{itemize}[leftmargin=*,itemsep=1pt]
\item It leaves the $\bigstar_1$ guard's representative bug in place. When
  \code{tc\_rel[0]} is not its own representative, $\bigstar_1$ does not fire, so
  \fn{isTCReachable} sees no graph during Phase~A and \fn{applyTCRule} emits a
  \textsc{TClos Down} splitting lemma for a membership that the graph would have
  discharged.
\item It keeps two call sites and a four-clause condition to reason about,
  where the up-front pass reduces the invariant to ``built once, before any
  rule, then only added to''.
\item It saves nothing: Phase~B already visits every closure term, so the
  up-front pass does no extra work.
\end{itemize}

\section{A merge hazard, measured}\label{sec:mergehazard}

The two fixes are individually sufficient and jointly \emph{not} composable in
the obvious way. Rebasing the Phase~0 patch onto \code{c851f83bf} produces a
textual conflict in exactly one place: the Phase~B \textsc{tclosure} branch,
which the patch deletes and which \#12793 has meanwhile rewritten. Resolving it
by keeping \#12793's branch and dropping only its \emph{guard} --- the natural
reading of ``Phase~0 already built everything, so the guard is redundant'' ---
restores the destructive call in full, and the resulting solver is exactly as
divergent as \code{8dd659a3a}:

\begin{center}\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lccc@{}}
\toprule
Benchmark & \emph{guarded} & Phase~0 \emph{plus} unguarded $\bigstar_2$ & \emph{once}\\
\midrule
\code{tc-overwrite-bug.smt2} & unsat & \textbf{timeout} & unsat\\
A \code{repro}               & unsat & \textbf{timeout} & unsat\\
E \code{chain4}              & unsat & \textbf{timeout} & unsat\\
\code{rel\_tc\_7.cvc.smt2}   & unsat & \textbf{timeout} & unsat\\
\bottomrule
\end{tabular}
\end{center}

\noindent
Measured on a Debug build with a 15\,s wall limit; the three controls
(B, C, D) answer correctly in all three configurations, which is what makes the
diagnosis specific. The reason the redundancy argument fails is
Observation~\ref{obs:destroy}: the second call is not idempotent with respect to
the first, because \fn{applyTCRule} has run in between. A Phase~0 pass makes the
\emph{first} build unnecessary, never the overwrite harmless.

\section{Measurement versus the unguarded code}\label{sec:measure}

\subsection{Setup}

This section compares \emph{once} against the \emph{unguarded} configuration,
which is what \code{main} looked like when the investigation started;
Section~\ref{sec:measure-vs-guarded} repeats the exercise against the
\emph{guarded} \code{main} that exists today.

Both configurations were built from \code{8dd659a3a} (branch \code{main}) with
the repository's existing \code{build/} tree
...
All 295 benchmarks under \file{test/regress/cli/*/rels/} and
\file{test/regress/cli/*/sets/}, 30\,s timeout each, expected results taken
from the \code{; EXPECT:} headers.

\begin{center}\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lcc@{}}
\toprule
& two calls (\code{main}) & once, up front\\
\midrule
benchmarks run                       & 295 & 295\\
answers differing between the two    & \multicolumn{2}{c}{1 (\code{rel\_tc\_7.cvc.smt2})}\\
disagreements with \code{; EXPECT:}  & 2   & \textbf{1}\\
correctness regressions              & \multicolumn{2}{c}{\textbf{0}}\\
correctness improvements             & \multicolumn{2}{c}{\textbf{1}}\\
total wall time                      & 125.6\,s & \textbf{96.8\,s}\\
total wall time, excluding the 4 timeouts & 5.6\,s & 6.7\,s\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
\item \textbf{Exactly one benchmark changes answer}, and it changes from
  \textsf{timeout} to \textsf{unsat}: the disabled \code{rel\_tc\_7} of
  Section~\ref{sec:reltc7}. Every other benchmark returns the same answer.
\item The remaining disagreement with \code{; EXPECT:} in both configurations is
  \code{regress1/rels/garbage\_collect.cvc.smt2}, which is in
  \code{regression\_disabled\_tests} and times out in this Debug build either
  way ($>180$\,s).
\item The 29\,s difference in total wall time is entirely the \code{rel\_tc\_7}
  timeout. Excluding all timeouts, the two configurations are within
  $\pm 1.1$\,s of each other over 291 benchmarks --- i.e.\ inside the noise of a
  Debug build. The saved second pass over the members is real but too small to
  measure here; the win is not a speedup, it is not diverging.
\item Per-inference statistics (\code{--stats-all},
  \code{theory::sets::inferencesLemma}) are \emph{identical} on all 18 in-tree
  benchmarks that mention \code{rel.tclosure} and that both configurations
  solve. The patch does not perturb inference behaviour where the overwrite was
  not destroying anything.
\end{itemize}

\subsection{Differential fuzzing against an exact oracle}

The in-tree suite barely exercises the failing combination, so a targeted
random test was added. Formulas have the shape
\begin{center}\small
$\{\tup{u_i,v_i}\in x\}_{i\in I}\;\cup\;\{\tup{u_j,v_j}\in\tc{x}\}_{j\in J}
 \;\cup\; \{\tup{a,b}\notin\tc{x}\}$
\end{center}
over pairwise-distinct integer constants, with $2$--$5$ nodes and $1$--$5$
...
\]
which a five-line depth-first search decides.

\paragraph{Result} 250 random instances, seed 7, 8\,s limit each; 202 of them
mention both an $x$-edge and a $\tc{x}$-edge.
...
& two calls (\code{main}) & once, up front\\
\midrule
matches the oracle       & 235 & \textbf{250}\\
timeout                  & \textbf{15}  & 0\\
\emph{wrong answer}      & \textbf{0}   & \textbf{0}\\
\midrule
answers differing between the two & \multicolumn{2}{c}{15}\\
\quad of which: \code{main} times out, once solves & \multicolumn{2}{c}{15}\\
\quad of which: once times out, \code{main} solves & \multicolumn{2}{c}{0}\\
\bottomrule
\end{tabular}
\end{center}

\noindent
Every divergence is in the same direction: current \code{main} fails to refute
an unsatisfiable instance that the one-call configuration refutes in
milliseconds. Critically, \textbf{neither configuration ever produced a wrong
answer} --- in particular the extra inferences enabled by the patch never turned
a satisfiable instance into \textsf{unsat}, which is the failure mode one would
worry about when strengthening a rule. A representative instance
($1\to 2$ and $0\to 1$ in $x$, $2\to 0$ in $\tc{x}$, so
$1\to 2\to 0\to 1$ and $\tup{1,0}\in\tc{x}$ is forced):

\begin{lstlisting}[style=smt]
(set-logic ALL)
(declare-fun x () (Relation Int Int))
(assert (set.member (tuple 0 1) x))
(assert (set.member (tuple 2 0) (rel.tclosure x)))
(assert (set.member (tuple 2 1) (rel.tclosure x)))
(assert (set.member (tuple 1 2) x))
(assert (not (set.member (tuple 1 0) (rel.tclosure x))))
(check-sat)          ; main: timeout      once: unsat
\end{lstlisting}

\section{Measurement versus the guarded code}\label{sec:measure-vs-guarded}

Once PR~\#12793 landed, the interesting question changed. The divergences of
Section~\ref{sec:measure} are gone from \code{main} on their own, so what has to
be shown is the opposite of a win: that replacing the guards with Phase~0
changes \emph{nothing} observable. Both binaries here were built from
\code{5cc03f4b9} (\code{main}, which contains \code{c851f83bf}) in the same
Debug tree, snapshotted as described in the sidenote above.
...
\begin{tabular}{@{}lccc@{}}
\toprule
Family, $N=64$ & \emph{guarded} & \emph{once} & ratio\\
\midrule
...
\subsection{The one case where \emph{once} could do more work}

\emph{once} builds one graph per closure \emph{term}; \emph{guarded} builds at
most one per base \emph{representative} (Section~\ref{sec:12793}). Where several
closure terms sit over provably equal base relations, \emph{once} therefore
populates \code{d\_tcr\_tcGraph} for all of them and \fn{doTCInference} walks
each --- $k$ times the forward inferences instead of one. Two probes bound the
risk:
...
recovers exactly \emph{guarded}'s work profile without reintroducing a second
call site.

\subsection{Differential fuzzing, repeated}

The oracle-based fuzzer of Section~\ref{sec:measure} was re-run against the
guarded baseline, with the generator widened to $2$--$7$ nodes and $1$--$10$
edges: 900 instances over seeds 1--3.

\begin{center}\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lcc@{}}
\toprule
& \emph{guarded} & \emph{once}\\
\midrule
...
\noindent
Both configurations now decide every instance the oracle decides. That is the
expected outcome, and it is worth recording that it is also a check on
\#12793: the guard fixes the same family of formulas the up-front pass does.

\section{Conclusion of the investigation}

\textbf{Building once, immediately after \fn{collectRelsInfo}, is better} --- but
the argument has to be stated against the right baseline, and it is no longer an
argument about divergence, because PR~\#12793 has already removed that. Against
the \emph{guarded} \code{main} the measurements say plainly: same answers, same
inference histograms, same run times to within noise
(Section~\ref{sec:measure-vs-guarded}). What remains is structural, and it is
...
\item \textbf{The invariant becomes checkable by inspection.}
  ...
  every closure term. \emph{guarded} establishes something weaker (per base
...
\item \textbf{It removes the coupling of correctness to bookkeeping state.}
  ...
  ``already built'' markers. Section~\ref{sec:mergehazard} is the concrete cost
  of that coupling: the guard is invisible protection, so the next person to
  touch Phase~B can delete it as redundant and reintroduce a divergence, which
  is what happened while this branch was being rebased.
\item \textbf{It removes a latent representative bug} in the $\bigstar_1$ guard
  (Section~\ref{sec:tcrule}), and stops \fn{buildTCGraphForRel} from mutating the
  membership maps through \code{operator[]}.
\item \textbf{Against the pre-\#12793 code it is also a large correctness win},
  which is what Section~\ref{sec:measure} measured and what
  \code{rel\_tc\_7.cvc.smt2} demonstrates; that test is re-enabled here.
\end{enumerate}

\noindent
Not claimed: a speedup. The second pass over the members is pure waste, and
removing waste is the right default, but at the sizes the relational solver
handles it is unmeasurable against the cost of the lemmas themselves.
...
at commit \code{8bcdf3239} (branch \code{build-trc-graph-once}), except where a
passage explicitly discusses the earlier \code{8dd659a3a} or \code{c851f83bf}
code.
...
# Against 8dd659a3a (unguarded): the divergence is the visible signal.
/tmp/base/cvc5 --tlimit=20000 A-repro.smt2   # -> timeout
/tmp/once/cvc5 --tlimit=20000 A-repro.smt2   # -> unsat

# Against c851f83bf (guarded): both answer everything, so compare the answers
# *and* the per-InferenceId histograms, which is the sensitive check.
python3 rels-guide/benchmarks/gen_stress.py /tmp/stress   # 47 + 32 benchmarks
for f in test/regress/cli/regress[01]/rels/*.smt2; do
