...
InferenceManager::InferenceManager(TheoryBagsPrivate& p,
                                   SolverState& s,
                                   eq::EqualityEngine& e,
                                   context::Context* c,
...
bool InferenceManager::assertFactRec(Node fact, Node exp, int inferType)
{
  ...
  if (fact.isConst())
  {
    ...
    if (fact == d_false)
    {
      Trace("bags-lemma") << "Conflict : " << exp << std::endl;
  ...
  if (atom.getKind() == MEMBER
      || (atom.getKind() == EQUAL && atom[0].getType().isBag()))
  {
...
void InferenceManager::flushLemma(Node lem, bool preprocess)
{
  ...
  {
    Trace("bags-lemma-debug") << "Already sent lemma : " << lem << std::endl;
    return;
  }
  Trace("bags-lemma-debug") << "Send lemma : " << lem << std::endl;
